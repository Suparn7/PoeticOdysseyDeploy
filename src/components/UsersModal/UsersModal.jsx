...
const UsersModal = ({ isOpen, onClose }) => {
  ...
  const userId = useState(user?.username)
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        setUsers(usersData.users.filter((doc) => doc.userId !== userId[0]));
      } catch (error) {
    ...
    const fetchPosts = async () => {
      if (fetchUserPosts) {
        ...
        try {
          ...
          const postsData = await dynamoService.getPostsByIds(fetchUserPosts.postsCreated);
          setPosts(postsData);
  ...
  const handleViewProfileClick = async (user) => {
    ...
    setLoading(true);
    try {
      const profileData = await dynamoUserInformationService.getUserInfoByUserNameId(user.userId); 
          ...
          {loading ? (
            ...
            <div className="user-profile-container">
              ...
              <div className="profile-body">
                ...
                <Link to={`/PoeticOdyssey/profile/${fetchUserProfile.userId}`}>
                    <button>
